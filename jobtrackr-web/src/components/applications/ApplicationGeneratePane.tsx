	...
	return (
		<div className="rounded-lg border border-light-gray bg-off-white">
			...
			<div
				id={panelId}
				className={cn(
					"grid transition-[grid-template-rows] max-w-full duration-300 ease-out motion-reduce:transition-none",
			...
			>
				<div className="min-h-0 max-w-full overflow-hidden">
					<div className="border-t border-light-gray px-3 pb-3 pt-3">
						...
						<createFetcher.Form
							...
							className="grid min-w-0 max-w-full gap-4 md:min-w-[auto] md:max-w-none [&>*]:min-w-0 md:[&>*]:min-w-[auto]"
							noValidate
							onSubmit={onSubmit}
						>
							...
							<div className="grid gap-2">
								...
								>
									<SelectTrigger
										id={`${formId}-base-cv`}
										aria-label="Base CV"
										className="min-w-0 max-w-full md:min-w-[auto] md:max-w-[100cqw]"
									>
...
export function ApplicationGeneratePane({ loading = false }: { loading?: boolean }) {
	...
	// Always scoped to the current Application so a kept-alive pane cannot show
	...
	const scopedRouteData =
		routeData?.applicationId === applicationId ? routeData : undefined;
	const scopedRefreshData =
		refreshFetcher.data?.applicationId === applicationId ? refreshFetcher.data : undefined;

	if (scopedRouteData) {
		retainedDataRef.current = scopedRouteData;
	} else if (scopedRefreshData) {
		retainedDataRef.current = scopedRefreshData;
	}

	const data = scopedRouteData ?? scopedRefreshData ?? retainedDataRef.current;
	...
	return (
		<div className="flex min-h-0 flex-col gap-3 p-1 pb-4">
			{activeGeneration ? (
